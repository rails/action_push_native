  ...
  class NotificationDeliveryJob < ActiveJob::Base
    ...
    with_options retry_options do
      ...
      with_options attempts: 20 do
        retry_on SocketError, Errno::ECONNREFUSED, Errno::ECONNRESET
    ...
    # Retry only SSL_connect errors.
    rescue_from OpenSSL::SSL::SSLError do |error|
      attempts = 3
      if executions_for(error) <= attempts && error.message.include?("SSL_connect")
        Rails.error.report(error) if ActionNativePush.configuration.report_job_retries
