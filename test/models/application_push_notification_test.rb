...
class ApplicationPushNotificationTest < ActiveSupport::TestCase
  test "deliver_later_to with custom queue" do
    ...
    assert_enqueued_with job: ApplicationPushNotificationJob, args: [ "ApplicationPushNotification", notification.as_json, action_push_native_devices(:pixel9) ], queue: :realtime
    assert_enqueued_with job: ApplicationPushNotificationJob, args: [ "ApplicationPushNotification", notification.as_json, action_push_native_devices(:iphone) ], queue: :realtime
  end

  test "deliver_to before_delivery callback" do
    notification = AbortablePushNotification.new(abort_delivery: true)
    ActionPushNative::Service::Apns.any_instance.expects(:push).never
    notification.deliver_to(action_push_native_devices(:iphone))

    notification = AbortablePushNotification.new(abort_delivery: false)
    ActionPushNative::Service::Apns.any_instance.stubs(:push)
    notification.deliver_to(action_push_native_devices(:iphone))
  end
