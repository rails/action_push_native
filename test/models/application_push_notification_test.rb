...
class ApplicationPushNotificationTest < ActiveSupport::TestCase
  test "deliver_later_to with custom queue" do
    ...
    assert_enqueued_with job: ApplicationPushNotificationJob, args: [ "ApplicationPushNotification", notification.as_json, action_native_push_devices(:pixel9) ], queue: :realtime
    assert_enqueued_with job: ApplicationPushNotificationJob, args: [ "ApplicationPushNotification", notification.as_json, action_native_push_devices(:iphone) ], queue: :realtime
  end
  ...
  test "deliver_to before_delivery callback" do
    ...
    notification.deliver_to(action_native_push_devices(:iphone))
